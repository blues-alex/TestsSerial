...
def main():
    sendCommand(f'$SM2{ENQ}')
    ba = "\x1f".join(['10000']*12)
    print(sendCommand(f"$SC01\x1f{ba}{ENQ}"))
    channels = [10000] * 12
    if len(sys.argv) > 1:
        for n, c in enumerate(sys.argv[1:]):
            channels[n] = int(10000 - float(c) * 100)
        mess = "\x1f".join([str(i) for i in channels])
        print(sendCommand(f"$SC01\x1f{mess}{ENQ}"))
        time.sleep(15)
        sendCommand(f'$SM1{ENQ}')
